...
const singleTree = (scene, shadowGenerator) => {
	...
	const bark = new BABYLON.StandardMaterial("bark", scene);
	bark.emissiveTexture = new BABYLON.Texture("textures/bark.jpg", scene);
	bark.diffuseTexture = new BABYLON.Texture("textures/bark.jpg", scene);
	...
	const branches = 8 + getrnd(2);
	const leaves_on_branch = 5;
    const leaf_wh_ratio = 0.5;
                
	const treeFull = createTree(
        trunk_height,
        trunk_taper,
        trunk_slices,
        bark,
...
//The code below is copied from https://github.com/BabylonJS/Extensions/blob/master/TreeGenerators/SPSTreeGenerator/TreeGenerator.js as is, as recommended by babylonjs docs
